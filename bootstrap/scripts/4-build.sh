...
${VM} "${PHARO_IMAGE_NAME}.image" "${IMAGE_FLAGS}" eval --save "(MetacelloProjectRegistration registry baselineProjectSpecs select: [ :spec | spec  repositories printString includesSubstring: 'github://' ]) asString."
${VM} "${PHARO_IMAGE_NAME}.image" "${IMAGE_FLAGS}" eval --save "MetacelloProjectRegistration registry baselineProjectSpecs asString."
