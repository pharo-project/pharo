...
${VM} "${COMPILER_IMAGE_NAME}.image" "${IMAGE_FLAGS}" bootstrap perform  --save ChronologyConstants initialize
${VM} "${COMPILER_IMAGE_NAME}.image" "${IMAGE_FLAGS}" bootstrap perform  --save DateAndTime initialize
${VM} "${COMPILER_IMAGE_NAME}.image" "${IMAGE_FLAGS}" bootstrap perform  --save SystemVersion setMajor:minor:patch:suffix:build:commitHash: ${PHARO_MAJOR} ${PHARO_MINOR} ${PHARO_PATCH} ${PHARO_SUFFIX} ${BUILD_NUMBER} ${PHARO_COMMIT_HASH}
...
${VM} "${COMPILER_IMAGE_NAME}.image" "${IMAGE_FLAGS}" bootstrap perform --save PharoBootstrapFixMethodsTool fixMethodsIn: protocolsKernel.txt
