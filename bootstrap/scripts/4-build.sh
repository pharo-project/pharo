...
${VM} "${PHARO_IMAGE_NAME}.image" "${IMAGE_FLAGS}" eval --save "((MetacelloProjectRegistration registry baselineProjectSpecs select: [ :spec | spec  repositories printString includes: 'github://'  ]) collect: [ :spec | { spec name . spec  className . spec repositories map anyOne description   } ]) asString"
