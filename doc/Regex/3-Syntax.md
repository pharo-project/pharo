...
## Repetition (Quantifiers and Pattern Arity)

Quantifiers control **how many times** the preceding element (character, character class, or group) must occur for a match.

| Quantifier | Meaning                                         | Example   | Matches                         | Does Not Match    |
|------------|-------------------------------------------------|-----------|----------------------------------|-------------------|
| `*`        | Zero or more occurrences                        | `ab*`     | `"a"`, `"ab"`, `"abb"`, `"abbb"` | `"b"`             |
| `+`        | One or more occurrences                         | `ab+`     | `"ab"`, `"abb"`, `"abbb"`        | `"a"`             |
| `?`        | Zero or one occurrence                          | `ab?`     | `"a"`, `"ab"`                    | `"abb"`           |
| `{n}`      | Exactly *n* occurrences                         | `a{3}`    | `"aaa"`                          | `"aa"`, `"aaaa"`  |
| `{n,}`     | *n* or more occurrences                         | `a{2,}`   | `"aa"`, `"aaa"`, `"aaaaa"`       | `"a"`             |
| `{n,m}`    | Between *n* and *m* occurrences (inclusive)     | `a{1,3}`  | `"a"`, `"aa"`, `"aaa"`           | `""`, `"aaaa"`    |

### Examples

```smalltalk
'aaa' matchesRegex: 'a*'.       "true — zero or more a’s"
'aaa' matchesRegex: 'a+'.       "true — one or more a’s"
'a'   matchesRegex: 'a?'.       "true — zero or one a"
'aaa' matchesRegex: 'a{3}'.     "true — exactly three a’s"
'aaaa' matchesRegex: 'a{2,}'.   "true — at least two a’s"
'aa'  matchesRegex: 'a{1,3}'.   "true — one to three a’s"
```

Quantifiers can be applied to:

- **Single characters**: `x+` → `"x"`, `"xx"`, `"xxx"`
- **Character classes**: `[0-9]{2,4}` → `"12"`, `"1234"`, `"567"`
- **Grouped patterns**: `(ab){2}` → `"abab"`

> **Tip:** The `*`, `+`, and `?` quantifiers are shorthand for range quantifiers:
> - `*` = `{0,}`
> - `+` = `{1,}`
> - `?` = `{0,1}`
...
### Selector-based Classes (Pharo specific)
...
If you want to specify a patter that is repeated
